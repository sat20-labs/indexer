...
func (m *medianTime) AddTimeSample(sourceID string, timeVal time.Time) {
	...
	common.Log.Debugf("Added time sample of %v (total: %v)", offsetDuration,
		numOffsets)
	...
	if math.Abs(float64(median)) < maxAllowedOffsetSecs {
		...
		if !m.invalidTimeChecked {
			...
			if !remoteHasCloseTime {
				common.Log.Warnf("Please check your date and time " +
	...
	common.Log.Debugf("New time offset: %v", medianDuration)
}
