...
func calcNextRequiredDifficulty(lastNode HeaderCtx, newBlockTime time.Time,
	...
	common.Log.Debugf("Difficulty retarget at block height %d", lastNode.Height()+1)
	common.Log.Debugf("Old target %08x (%064x)", lastNode.Bits(), oldTarget)
	common.Log.Debugf("New target %08x (%064x)", newTargetBits, CompactToBig(newTargetBits))
	common.Log.Debugf("Actual timespan %v, adjusted timespan %v, target timespan %v",
