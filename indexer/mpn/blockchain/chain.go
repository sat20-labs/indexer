...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockLocatorFromHash(hash *chainhash.Hash) utils.BlockLocator {
	b.chainLock.RLock()
	node := b.index.LookupNode(hash)
...
//
// This function is safe for concurrent access.
func (b *BlockChain) LatestBlockLocator() (utils.BlockLocator, error) {
	b.chainLock.RLock()
...
//
// This function MUST be called with the chain state lock held (for reads).
func (b *BlockChain) locateInventory(locator utils.BlockLocator, hashStop *chainhash.Hash, maxEntries uint32) (*blockNode, uint32) {
...
//
// This function MUST be called with the chain state lock held (for reads).
func (b *BlockChain) locateBlocks(locator utils.BlockLocator, hashStop *chainhash.Hash, maxHashes uint32) []chainhash.Hash {
...
//
// This function MUST be called with the chain state lock held (for reads).
func (b *BlockChain) locateHeaders(locator utils.BlockLocator, hashStop *chainhash.Hash, maxHeaders uint32) []wire.BlockHeader {
	// Find the node after the first known block in the locator and the
	// total number of nodes after it needed while respecting the stop hash
	// and max entries.
...
//
// This function is safe for concurrent access.
func (b *BlockChain) LocateHeaders(locator utils.BlockLocator, hashStop *chainhash.Hash) []wire.BlockHeader {
	b.chainLock.RLock()
...
func New(config *Config) (*BlockChain, error) {
	...
	// 	return nil, err
	// }
	// common.Log.Infof("Chain state (height %d, hash %v, totaltx %d)",
