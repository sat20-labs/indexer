...
func checkInputsStandard(tx *btcutil.Tx, utxoView *common.UtxoViewpoint) error {
...
func GetDustThreshold(txOut *wire.TxOut) int64 {
	...
	if txscript.IsWitnessProgram(txOut.PkScript) {
		totalSize += (107 / common.WitnessScaleFactor)
	} else {
		totalSize += 107
	}
...
func CheckTransactionStandard(tx *btcutil.Tx, height int32,
	...
	if !common.IsFinalizedTransaction(tx, height, medianTimePast) {
		return txRuleError(wire.RejectNonstandard,
	...
	// attacks.
	txWeight := common.GetTransactionWeight(tx)
...
func GetTxVirtualSize(tx *btcutil.Tx) int64 {
	...
	return (common.GetTransactionWeight(tx) + (common.WitnessScaleFactor - 1)) /
	common.WitnessScaleFactor
}
