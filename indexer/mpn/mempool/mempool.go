...
func (mp *TxPool) addOrphan(tx *btcutil.Tx, tag Tag) {
	...
	common.Log.Debugf("Stored orphan transaction %v (total: %d)", tx.Hash(),
		len(mp.orphans))
}
...
func (mp *TxPool) maybeAcceptTransaction(tx *btcutil.Tx, isNew, rateLimit,
	...
	for _, conflict := range r.Conflicts {
		common.Log.Debugf("Replacing transaction %v (fee_rate=%v sat/kb) "+
	...
	common.Log.Debugf("Accepted transaction %v (pool size: %v)", txHash,
		len(mp.pool))
...
func (mp *TxPool) ProcessTransaction(tx *btcutil.Tx, allowOrphan, rateLimit bool, tag Tag) ([]*TxDesc, error) {
	common.Log.Tracef("Processing transaction %v", tx.Hash())
...
func (mp *TxPool) CheckMempoolAcceptance(tx *btcutil.Tx) (
	*MempoolAcceptResult, error) {
	...
	if err != nil {
		common.Log.Errorf("CheckMempoolAcceptance: %v", err)
		return nil, err
	}

	common.Log.Tracef("Tx %v passed mempool acceptance check: %v", tx.Hash(),
		spew.Sdump(result))
...
func (mp *TxPool) checkMempoolAcceptance(tx *btcutil.Tx,
	...
	// Exit early if this transaction is missing parents.
	if len(missingParents) > 0 {
		common.Log.Debugf("Tx %v is an orphan with missing parents: %v",
...
func (mp *TxPool) validateRelayFeeMet(tx *btcutil.Tx, txFee, txSize int64,
	...
	common.Log.Tracef("rate limit: curTotal %v, nextTotal: %v, limit %v",
