...
func (cm *ConnManager) handleFailedConn(c *ConnReq) {
	...
	if c.Permanent {
		...
		common.Log.Debugf("Retrying connection to %v in %v", c, d)
		...
		if cm.failedAttempts >= maxFailedAttempts {
			common.Log.Debugf("Max failed connection attempts reached: [%d] "+
...
out:
		...
		case req := <-cm.requests:
			...
			case handleConnected:
				...
				if _, ok := pending[connReq.id]; !ok {
					...
					common.Log.Debugf("Ignoring connection for "+
				...
				common.Log.Debugf("Connected to %v", connReq)
			...
			case handleDisconnected:
				...
				if !ok {
					...
					if !ok {
						common.Log.Errorf("Unknown connid=%d",
							msg.id)
						continue
					}
					...
					common.Log.Debugf("Canceling: %v", connReq)
				...
				// callback.
				common.Log.Debugf("Disconnected from %v", connReq)
				delete(conns, msg.id)
				...
				if uint32(len(conns)) < cm.cfg.TargetOutbound ||
					...
					connReq.updateState(ConnPending)
					common.Log.Debugf("Reconnecting to %v",
						connReq)
			...
			case handleFailed:
				...
				common.Log.Debugf("Failed to connect to %v: %v",
					connReq, msg.err)
				cm.handleFailedConn(connReq)
			}
	...
	cm.wg.Done()
	common.Log.Trace("Connection handler done")
}
...
func (cm *ConnManager) Connect(c *ConnReq) {
	...
	if c.State() == ConnCanceled {
		common.Log.Debugf("Ignoring connect for canceled connreq=%v", c)
		return
	}
	...
	common.Log.Debugf("Attempting to connect to %v", c)
...
func (cm *ConnManager) listenHandler(listener net.Listener) {
	common.Log.Infof("Server listening on %s", listener.Addr())
	for atomic.LoadInt32(&cm.stop) == 0 {
		conn, err := listener.Accept()
		if err != nil {
			// Only common.Log the error if not forcibly shutting down.
			if atomic.LoadInt32(&cm.stop) == 0 {
				common.Log.Errorf("Can't accept connection: %v", err)
			}
			continue
		}
	...
	cm.wg.Done()
	common.Log.Tracef("Listener handler done for %s", listener.Addr())
}
...
func (cm *ConnManager) Start() {
	...
	common.Log.Trace("Connection manager started")
	cm.wg.Add(1)
	go cm.connHandler()
...
func (cm *ConnManager) Stop() {
	if atomic.AddInt32(&cm.stop, 1) != 1 {
		common.Log.Warnf("Connection manager already stopped")
		return
	}
	...
	close(cm.quit)
	common.Log.Trace("Connection manager stopped")
}
