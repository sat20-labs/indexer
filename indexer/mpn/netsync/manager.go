...
func (sm *SyncManager) startSync() {
	...
	if err != nil {
		common.Log.Errorf("Unable to query for segwit soft-fork state: %v", err)
		return
	}
	...
	for peer, state := range sm.peerStates {
		...
		if segwitActive && !peer.IsWitnessEnabled() {
			common.Log.Debugf("peer %v not witness enabled, skipping", peer)
			continue
		}
	...
	if bestPeer != nil {
		...
		if err != nil {
			common.Log.Errorf("Failed to get block locator for the "+
				"latest block: %v", err)
			return
		}

		common.Log.Infof("Syncing to block height %d from peer %v",
		...
		if sm.nextCheckpoint != nil &&
			...
			common.Log.Infof("Downloading headers for blocks %d to "+
		...
		common.Log.Warnf("No sync peer candidates available")
	}
}
...
func (sm *SyncManager) handleNewPeerMsg(peer *peerpkg.Peer) {
	...
	common.Log.Infof("New valid peer %s (%s)", peer, peer.UserAgent())
...
func (sm *SyncManager) handleDonePeerMsg(peer *peerpkg.Peer) {
	...
	if !exists {
		common.Log.Warnf("Received done peer message for unknown peer %s", peer)
		return
	}
	...
	common.Log.Infof("Lost peer %s", peer)
...
func (sm *SyncManager) updateSyncPeer(dcSyncPeer bool) {
	common.Log.Debugf("Updating sync peer, no progress for: %v",
...
func (sm *SyncManager) handleTxMsg(tmsg *txMsg) {
	...
	common.Log.Infof("Received New TX %s (%s)", tmsg.peer, tmsg.tx.Hash().String())

	peer := tmsg.peer
	state, exists := sm.peerStates[peer]
	if !exists {
		common.Log.Warnf("Received tx message from unknown peer %s", peer)
		return
	}
	...
	if _, exists = sm.rejectedTxns[*txHash]; exists {
		common.Log.Debugf("Ignoring unsolicited previously rejected "+
	...
	if err != nil {
		...
		// so common.Log it as such.  Otherwise, something really did go wrong,
		// so common.Log it as an actual error.
		if _, ok := err.(mempool.RuleError); ok {
			common.Log.Debugf("Rejected transaction %v from %s: %v",
				txHash, peer, err)
		} else {
			common.Log.Errorf("Failed to process transaction %v: %v",
				txHash, err)
		}
...
func (sm *SyncManager) handleBlockMsg(bmsg *blockMsg) {
	common.Log.Infof("Received block %d from peer %s", bmsg.block.Height(), bmsg.peer)

	peer := bmsg.peer
	state, exists := sm.peerStates[peer]
	if !exists {
		common.Log.Warnf("Received block message from unknown peer %s", peer)
		return
	}
	...
	if _, exists = state.requestedBlocks[*blockHash]; !exists {
		...
		// duplicate blocks.
		if sm.chainParams != &chaincfg.RegressionNetParams {
			common.Log.Warnf("Got unrequested block %v from %s -- "+
	...
	if err != nil {
		...
		if _, ok := err.(blockchain.RuleError); ok {
			...
			common.Log.Errorf("Failed to process block %v: %v",
				blockHash, err)
		}
	...
	if isOrphan {
		...
		// When the block is not an orphan, common.Log information about it and
	...
	if sm.nextCheckpoint != nil {
		...
		common.Log.Infof("Downloading headers for blocks %d to %d from "+
...
func (sm *SyncManager) fetchHeaderBlocks() {
	...
	if sm.startHeader == nil {
		common.Log.Warnf("fetchHeaderBlocks called with no start header")
		return
	}
	...
	for e := sm.startHeader; e != nil; e = e.Next() {
		...
		if !ok {
			common.Log.Warn("Header list node type is not a headerNode")
			continue
		}
		...
		if err != nil {
			common.Log.Warnf("Unexpected failure when checking for "+
...
func (sm *SyncManager) handleHeadersMsg(hmsg *headersMsg) {
	common.Log.Infof("Received headers message from peer %s", hmsg.peer)
	peer := hmsg.peer
	_, exists := sm.peerStates[peer]
	if !exists {
		common.Log.Warnf("Received headers message from unknown peer %s", peer)
		return
	}
	...
	if !sm.headersFirstMode {
		common.Log.Warnf("Got %d unrequested headers from %s -- "+
	...
	for _, blockHeader := range msg.Headers {
		...
		if prevNodeEl == nil {
			common.Log.Warnf("Header list does not contain a previous" +
		...
		if prevNode.hash.IsEqual(&blockHeader.PrevBlock) {
			...
			common.Log.Warnf("Received block header that does not "+
		...
		if node.height == sm.nextCheckpoint.Height {
			if node.hash.IsEqual(sm.nextCheckpoint.Hash) {
				...
				common.Log.Infof("Verified downloaded block "+
					"header against checkpoint at height "+
					"%d/hash %s", node.height, node.hash)
			} else {
				common.Log.Warnf("Block header at height %d/hash "+
	...
	if receivedCheckpoint {
		...
		common.Log.Infof("Received %v block headers: Fetching blocks",
			sm.headerList.Len())
...
func (sm *SyncManager) handleNotFoundMsg(nfmsg *notFoundMsg) {
	common.Log.Infof("Received notfound message from peer %s", nfmsg.peer)
	peer := nfmsg.peer
	state, exists := sm.peerStates[peer]
	if !exists {
		common.Log.Warnf("Received notfound message from unknown peer %s", peer)
		return
	}
...
func (sm *SyncManager) handleInvMsg(imsg *invMsg) {
	common.Log.Infof("Received inv message from peer %s", imsg.peer)

	peer := imsg.peer
	state, exists := sm.peerStates[peer]
	if !exists {
		common.Log.Warnf("Received inv message from unknown peer %s", peer)
		return
	}
...
out:
	for {
		...
		case m := <-sm.msgChan:
			...
			default:
				common.Log.Warnf("Invalid message type in block "+
					"handler: %T", msg)
			}
...
func (sm *SyncManager) handleBlockchainNotification(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockAccepted:
		...
		common.Log.Infof("Chain accepted notification %d", block.Height())
	...
	case blockchain.NTBlockConnected:
		...
		common.Log.Infof("Chain connected notification %d", block.Height())
	...
	case blockchain.NTBlockDisconnected:
		...
		common.Log.Infof("Chain disconnected notification %d", block.Height())
...
func (sm *SyncManager) Start() {
	...
	common.Log.Trace("Starting sync manager")
	sm.wg.Add(1)
	go sm.blockHandler()
}
...
func (sm *SyncManager) Stop() error {
	if atomic.AddInt32(&sm.shutdown, 1) != 1 {
		common.Log.Warnf("Sync manager is already in the process of " +
			"shutting down")
		return nil
	}

	common.Log.Infof("Sync manager shutting down")
	close(sm.quit)
	sm.wg.Wait()
	return nil
}
...
func New(config *Config) (*SyncManager, error) {
	...
	if !config.DisableCheckpoints {
		...
		common.Log.Info("Checkpoints are disabled")
	}
