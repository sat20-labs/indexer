...
func (a *AddrManager) updateAddress(netAddr, srcAddr *wire.NetAddressV2) {
	...
	if len(a.addrNew[bucket]) > newBucketSize {
		common.Log.Tracef("new bucket is full, expiring old")
		a.expireNew(bucket)
	}
	...
	common.Log.Tracef("Added new address %s for a total of %d addresses", addr,
		a.nTried+a.nNew)
}
...
func (a *AddrManager) expireNew(bucket int) {
	...
	for k, v := range a.addrNew[bucket] {
		if v.isBad() {
			common.Log.Tracef("expiring bad address %v", k)
	...
	if oldest != nil {
		...
		common.Log.Tracef("expiring oldest address %v", key)
...
out:
	...
	a.savePeers()
	a.wg.Done()
	common.Log.Trace("Address handler done")
}
...
func (a *AddrManager) savePeers() {
	...
	if err != nil {
		common.Log.Errorf("Error opening file %s: %v", a.peersFile, err)
		return
	}
	enc := json.NewEncoder(w)
	defer w.Close()
	if err := enc.Encode(&sam); err != nil {
		common.Log.Errorf("Failed to encode file %s: %v", a.peersFile, err)
		return
	}
}
...
func (a *AddrManager) loadPeers() {
	...
	if err != nil {
		common.Log.Errorf("Failed to parse file %s: %v", a.peersFile, err)
		// if it is invalid we nuke the old one unconditionally.
		err = os.Remove(a.peersFile)
		if err != nil {
			common.Log.Warnf("Failed to remove corrupt peers file %s: %v",
				a.peersFile, err)
		}
		a.reset()
		return
	}
	common.Log.Infof("Loaded %d addresses from file '%s'", a.numAddresses(), a.peersFile)
}
...
func (a *AddrManager) Start() {
	...
	common.Log.Trace("Starting address manager")
...
func (a *AddrManager) Stop() error {
	if atomic.AddInt32(&a.shutdown, 1) != 1 {
		common.Log.Warnf("Address manager is already in the process of " +
			"shutting down")
		return nil
	}

	common.Log.Infof("Address manager shutting down")
	close(a.quit)
	a.wg.Wait()
	return nil
}
	...
	if a.nTried > 0 && (a.nNew == 0 || a.rand.Intn(2) == 0) {
		...
		for {
			...
			if float64(randval) < (factor * ka.chance() * float64(large)) {
				common.Log.Tracef("Selected %v from tried bucket",
					NetAddressKey(ka.na))
				return ka
			}
			factor *= 1.2
		...
		for {
			...
			randval := a.rand.Intn(large)
			if float64(randval) < (factor * ka.chance() * float64(large)) {
				common.Log.Tracef("Selected %v from new bucket",
					NetAddressKey(ka.na))
				return ka
			}
			factor *= 1.2
...
func (a *AddrManager) Good(addr *wire.NetAddressV2) {
	...
	common.Log.Tracef("Replacing %s with %s in tried", rmkey, addrKey)
...
func (a *AddrManager) GetBestLocalAddress(remoteAddr *wire.NetAddressV2) *wire.NetAddressV2 {
	...
	if bestAddress != nil {
		common.Log.Debugf("Suggesting address %s:%d for %s:%d",
			bestAddress.Addr.String(), bestAddress.Port,
			remoteAddr.Addr.String(), remoteAddr.Port)
	} else {
		common.Log.Debugf("No worthy address for %s:%d",
			remoteAddr.Addr.String(), remoteAddr.Port)
