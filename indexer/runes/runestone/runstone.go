...
func (r *Runestone) DecipherFromTx(transaction *wire.MsgTx) (*Artifact, error) {
	payload, err := r.payloadFromTx(transaction)
	if err != nil {
		if payload != nil {
			return &Artifact{
				Cenotaph: &Cenotaph{
	...
	message, _ := NewMessageFromTx2Payload(transaction, integers)
	flags, _ := TagTake(TagFlags, message.Fields,
	...
	if FlagEtching.Take(flags) {
		etching = &Etching{}
		etching.Divisibility, _ = TagTake(TagDivisibility, message.Fields,
		...
		etching.Premine, _ = TagTake(TagPremine, message.Fields,
			func(uint128s []uint128.Uint128) (*uint128.Uint128, error) {
				return &uint128s[0], nil
			}, 1)
		// rune: Tag::Rune.take(&mut fields, |[rune]| Some(Rune(rune))),
		etching.Rune, _ = TagTake(TagRune, message.Fields,
		...
		//      }),
		etching.Spacers, _ = TagTake(TagSpacers, message.Fields,
		...
		//      }),
		etching.Symbol, _ = TagTake(TagSymbol, message.Fields,
			func(uint128s []uint128.Uint128) (*rune, error) {
		...
		if FlagTerms.Take(flags) {
			terms := Terms{}
			terms.Cap, _ = TagTake(TagCap, message.Fields,
				func(uint128s []uint128.Uint128) (*uint128.Uint128, error) {
					return &uint128s[0], nil
				}, 1)
			terms.Height[0], _ = TagTake(TagHeightStart, message.Fields,
				func(uint128s []uint128.Uint128) (*uint64, error) {
					h := uint128s[0].Lo
					return &h, nil
				}, 1)
			terms.Height[1], _ = TagTake(TagHeightEnd, message.Fields,
				func(uint128s []uint128.Uint128) (*uint64, error) {
					h := uint128s[0].Lo
					return &h, nil
				}, 1)
			terms.Amount, _ = TagTake(TagAmount, message.Fields,
				func(uint128s []uint128.Uint128) (*uint128.Uint128, error) {
					return &uint128s[0], nil
				}, 1)
			terms.Offset[0], _ = TagTake(TagOffsetStart, message.Fields,
				func(uint128s []uint128.Uint128) (*uint64, error) {
					h := uint128s[0].Lo
					return &h, nil
				}, 1)
			terms.Offset[1], _ = TagTake(TagOffsetEnd, message.Fields,
				func(uint128s []uint128.Uint128) (*uint64, error) {
					h := uint128s[0].Lo
					return &h, nil
				}, 1)
	...
	//    });
	mint, _ := TagTake(TagMint, message.Fields,
	...
	//    });
	pointer, _ := TagTake(TagPointer, message.Fields,
		func(uint128s []uint128.Uint128) (*uint32, error) {
...
// Decipher2
func (r *Runestone) DecipherFromPkScript(pkScript []byte) (*Artifact, error) {
	payload, err := r.payloadFromPkScript(pkScript)
	if err != nil {
		if payload != nil {
			return &Artifact{
				Cenotaph: &Cenotaph{
	...
	message, _ := NewMessageFromPayload(integers)
	flags, _ := TagTake(TagFlags, message.Fields,
		func(uint128s []uint128.Uint128) (*uint128.Uint128, error) {
			return &uint128s[0], nil
		}, 1)
...
func (r *Runestone) payloadFromTx(transaction *wire.MsgTx) (*Payload, error) {
...
// payload2
func (r *Runestone) payloadFromPkScript(pkScript []byte) (*Payload, error) {
