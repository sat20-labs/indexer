...
func (p *NftIndexer) UpdateTransfer(block *common.Block, coinbase []*common.Range) {
		...
		if change != nil {
			...
			if newSize + change.Value() <= coinbaseSize {
				...
				if size > 0 {
					...
					if err != nil {
						common.Log.Panicf("NftIndexer.UpdateTransfer cut %s failed, %v", tx.TxId, err)
					}
	...
	// 	p.processCallback.TxInputProcess(i, tx, block, coinbase)
	// }
...
func (p *NftIndexer) PrepareUpdateTransfer(block *common.Block, coinbase []*common.Range) {
	...
	p.db.View(func(txn common.ReadBatch) error {
		...
		for _, v := range satLoadingVector {
			...
			if err != nil {
				common.Log.Panicf("block %d loadNftsInSatFromTxn sat %d failed, %v", block.Height, sat, err)
			}
