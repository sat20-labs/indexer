...
func (m *messageSenderImpl) SendRequest(ctx context.Context, p peer.ID, pmes *pb.Message) (*pb.Message, error) {
	...
	if err != nil {
		...
		logger.Debug("request failed to open message sender", "error", err, "to", p)
		return nil, err
	}
	...
	if err != nil {
		...
		logger.Debug("request failed", "error", err, "to", p)
		return nil, err
	}
...
func (m *messageSenderImpl) SendMessage(ctx context.Context, p peer.ID, pmes *pb.Message) error {
	...
	if err != nil {
		...
		logger.Debug("message failed to open message sender", "error", err, "to", p)
		return err
	}
	...
	if err := ms.SendMessage(ctx, pmes); err != nil {
		...
		logger.Debug("message failed", "error", err, "to", p)
		return err
	}
...
func (ms *peerMessageSender) SendMessage(ctx context.Context, pmes *pb.Message) error {
	...
	for {
		...
		if err := ms.writeMsg(pmes); err != nil {
			...
			if retry {
				logger.Debug("error writing message", "error", err)
				return err
			}
			logger.Debug("error writing message", "error", err, "retrying", true)
			retry = true
			continue
		}
...
func (ms *peerMessageSender) SendRequest(ctx context.Context, pmes *pb.Message) (*pb.Message, error) {
	...
	for {
		...
		if err := ms.ctxReadMsg(ctx, mes); err != nil {
			...
			if retry {
				logger.Debug("error reading message", "error", err)
				return nil, err
			}
			logger.Debug("error reading message", "error", err, "retrying", true)
			retry = true
			continue
		}
